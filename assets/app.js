...
(function () {
  ...
  function updateThemeLabel(t) {
    ...
    btn.innerHTML = icon + " <span class='vis-label'>" + t.charAt(0).toUpperCase() + t.slice(1) + "</span>";
  ...
  function refreshProgressUI() {
    ...
    document.querySelectorAll("[data-progress-text]").forEach(function (el) { el.textContent = doneCount + " / " + TOTAL_LESSONS + " lessons (" + pct + "%)"; });
    ...
    if (btn && thisSlug) {
      ...
      btn.textContent = isDone ? "✓ Marked complete" : "Mark this lesson complete";
    ...
    document.querySelectorAll(".tracer[data-trace]").forEach(function (el) {
      ...
      function renderStep(i) {
        ...
        memBox.innerHTML = Object.keys(mem).length
          ...
          : '<span style="color:var(--ink-faint);font-size:.85rem">(no variables yet)</span>';
        outBox.textContent = trace[idx].output || "";
        descBox.textContent = trace[idx].desc || "";
        stepOf.textContent = "Step " + (idx + 1) + " / " + trace.length;
        slider.value = idx;
      ...
      function stopPlay() { playing = false; clearInterval(timer); playBtn.textContent = "▶ Run"; }
      function startPlay() {
    ...
    document.querySelectorAll(".quiz").forEach(function (quiz) {
      ...
      if (submitBtn) {
        submitBtn.addEventListener("click", function () {
          ...
          if (scoreBox) {
            var verdict = pct >= 80 ? "Excellent! 🎉" : pct >= 50 ? "Good effort — review and retry." : "Keep practicing — review the lesson above.";
    ...
    document.querySelectorAll(".exercise-box[data-answer]").forEach(function (box) {
      ...
      if (checkBtn) {
        checkBtn.addEventListener("click", function () {
          ...
          feedback.textContent = isCorrect ? "Correct! " + (box.getAttribute("data-explain") || "") : "Not quite — try again, or check the hint.";
        });
      }
  ...
  function initRunners() {
    document.querySelectorAll("[data-runner]").forEach(function (wrap) {
      ...
      runBtn.addEventListener("click", function () {
        output.textContent = "Loading Python in your browser (first run only)…";
        ...
        loadPyodide_().then(function (pyodide) {
          ...
          try {
            ...
            output.innerHTML = escapeHtml_(capturedOut || "(no output)");
          } catch (err) {
            output.innerHTML = escapeHtml_(capturedOut) + '<span class="err">' + escapeHtml_(String(err)) + "</span>";
          }
          runBtn.disabled = false;
        }).catch(function (err) {
          output.innerHTML = '<span class="err">Could not load the in-browser Python engine (Pyodide). This needs an internet connection. ' + escapeHtml_(String(err)) + "</span>";
          runBtn.disabled = false;
        });
      });
